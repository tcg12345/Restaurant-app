...
const mapDbRestaurantToRestaurant = (dbRestaurant: DbRestaurant): Restaurant => ({
  ...
  googlePlaceId: dbRestaurant.google_place_id ?? undefined,
});
...
export function RestaurantProvider({ children }: RestaurantProviderProps) {
      ...
      .on(
        ...
        (payload) => {
          ...
          if (payload.eventType === 'INSERT' && payload.new) {
            ...
            console.log('🔄 Real-time INSERT detected for:', newRestaurant.name);
            setRestaurants(prev => {
              // Check if restaurant already exists to prevent duplicates
              const exists = prev.some(r => r.id === newRestaurant.id);
              if (exists) {
                console.log('⚠️ Restaurant already exists in local state, skipping real-time insert');
                return prev;
              }
              console.log('✅ Adding restaurant from real-time insert');
  ...
  const addRestaurant = useCallback(async (data: RestaurantFormData): Promise<string> => {
    ...
    console.log(`🚀 [${callId}] addRestaurant called with:`, {
      name: data.name,
    ...
    console.log(`🔍 [${callId}] Current restaurants count:`, restaurants.length);
    const existingRestaurant = restaurants.find(restaurant => {
      ...
      const placeIdMatch = restaurant.googlePlaceId === data.googlePlaceId;
      
      console.log(`🔍 [${callId}] Checking for duplicate - ${restaurant.name}:`, {
        nameMatch,
        addressMatch,
        placeIdMatch,
        ...
        dataPlaceId: data.googlePlaceId
      });
    ...
    if (existingRestaurant) {
      console.log(`❌ [${callId}] Duplicate restaurant found, throwing error`);
      throw new Error(`Restaurant "${data.name}" already exists in your ratings. Please edit the existing entry instead.`);
    }
    
    console.log(`✅ [${callId}] No duplicate found, proceeding with insert`);
    ...
    try {
      ...
      const newRestaurant = {
        ...
        google_place_id: (data as any).googlePlaceId ?? null,
      ...
      console.log('About to insert restaurant data:', { 
        ...
        google_place_id: newRestaurant.google_place_id
      });
      ...
      console.log(`💾 [${callId}] Inserted restaurant with ID:`, inserted.id);
      console.log(`📝 [${callId}] Updating local state with restaurant:`, restaurant.name);
      setRestaurants((prev) => {
        console.log(`📝 [${callId}] Previous restaurants count:`, prev.length);
        const newRestaurants = [restaurant, ...prev];
        console.log(`📝 [${callId}] New restaurants count:`, newRestaurants.length);
  ...
  }, [restaurants, uploadCompressedPhotos, geocodeAddress]);
...
const { data: updated, error } = await supabase
  ...
  const loadRestaurantPhotos = useCallback(async (id: string): Promise<void> => {
    try {
      // Try to load photos, but don't fail if the query doesn't work
      // Some restaurants might not have these columns or RLS might block access
      const { data, error } = await supabase
        .from('restaurants')
        .select('photos, photo_dish_names, photo_notes, photo_captions')
        .eq('id', id)
        .single();

      // If we get a 406 or other error, try with just photos column
      if (error) {
        // Try a simpler query with just photos
        ...
        if (simpleError) {
          // Silently fail - photos might not be available for this restaurant
          console.warn('Could not load photos for restaurant:', id, simpleError);
          return;
        }
      ...
      // Silently handle errors - photos are optional
      console.warn('Error loading restaurant photos:', error);
    }
  }, []);
