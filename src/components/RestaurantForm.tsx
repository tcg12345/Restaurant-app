...
export function RestaurantForm({ initialData, onSubmit, onCancel, defaultWishlist = false, hideSearch = false, defaultSelectedListId, allowPhotoReordering = true }: RestaurantFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const submissionData = {
      ...
      googlePlaceId: formDataWithPlaces.googlePlaceId ?? (initialData as any)?.googlePlaceId ?? null,
    ...
    console.log('📝 RestaurantForm submitting data:', {
      name: submissionData.name,
      googlePlaceId: submissionData.googlePlaceId,
      address: submissionData.address,
      isWishlist: submissionData.isWishlist
    });
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        {!formData.isWishlist && (
          <div className="space-y-6">
            <div className="flex items-center gap-2 mb-4">
              <div className="h-1 w-8 bg-primary rounded-full"></div>
              <h3 className="text-lg font-semibold">Rating & Experience</h3>
            </div>
              ...
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Label htmlFor="useWeightedRating" className="text-sm text-muted-foreground">
                      Use detailed rating system
                    </Label>
                    <Switch
                ...
                <div className="space-y-2">
                  <Label className="text-base font-medium">When did you visit?</Label>
                  <Popover>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-card/50 rounded-lg p-6 border border-border/50">
            <div className="space-y-3">
              <Label htmlFor="notes" className="text-base font-medium">Share your experience</Label>
              <Textarea
                id="notes"
                name="notes"
                value={formData.notes}
                onChange={handleInputChange}
                placeholder="Write your review or notes about this restaurant... What made it special? Any standout dishes or experiences?"
        ...
        {!formData.isWishlist && (
          <div className="space-y-6">
            <div className="flex items-center gap-2 mb-4">
              <div className="h-1 w-8 bg-primary rounded-full"></div>
              <h3 className="text-lg font-semibold">Photos & Memories</h3>
            </div>
            <div className="bg-card/50 rounded-lg p-6 border border-border/50">
              ...
              <div key={index} className="space-y-2">
                <div 
                  className={`group relative aspect-square overflow-hidden rounded-md border transition-all duration-200 ${
                    allowPhotoReordering && draggedPhotoIndex === index 
                      ? 'opacity-50 scale-95 cursor-grabbing' 
                      : allowPhotoReordering 
                        ? 'cursor-grab hover:shadow-lg'
                        : 'hover:shadow-lg'
                  }`}
                  draggable={allowPhotoReordering}
                  {...(allowPhotoReordering && {
                    onDragStart: (e) => {
        ...
        <div className="flex justify-end gap-3 pt-6 border-t border-border/50">
          ...
          >
            {formData.isWishlist ? "Add to Wishlist" : "Save Restaurant"}
          </Button>
        </div>
        </div>
      </form>
