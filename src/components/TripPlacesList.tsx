...
export function TripPlacesList({ 
  ...
  const [selectedPlaceForRating, setSelectedPlaceForRating] = useState<PlaceRating | null>(null);
  const { addRestaurant } = useRestaurants();
  const { toast } = useToast();

  const handleRatePlace = (place: PlaceRating, e: React.MouseEvent) => {
    e.stopPropagation();
    setSelectedPlaceForRating(place);
    setIsRatingBottomSheetOpen(true);
  };

  const handleRatingSubmit = (placeId: string, rating: number) => {
    if (!selectedPlaceForRating) return;

    // Convert PlaceRating to Restaurant format for adding to rated restaurants
    const ratedRestaurant = {
      name: selectedPlaceForRating.place_name,
      address: selectedPlaceForRating.address || '',
      city: '', // Not available in PlaceRating interface
      cuisine: selectedPlaceForRating.place_type || 'Restaurant',
      priceRange: selectedPlaceForRating.price_range || 1,
      rating: rating,
      reviewCount: 0,
      notes: `Rated ${rating}/10 from trip places`,
      isWishlist: false,
      photos: [] as File[],
      michelinStars: 0,
    };
    ...
    toast({
      title: "Place rated!",
      description: `${selectedPlaceForRating.place_name} has been rated ${rating}/10 and added to your ratings`,
    });
  ...
  const convertToRestaurant = (place: PlaceRating): Restaurant | null => {
    if (!place) return null;

    return {
      id: place.id, // Use place ID
      name: place.place_name,
      address: place.address || '',
      city: '', // Not available in PlaceRating
      cuisine: place.place_type || 'Restaurant',
  ...
  return (
            ...
            {displayMode === 'minimal' && (
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <Button
                  variant="default"
                  size="sm"
                  onClick={(e) => handleRatePlace(rating, e)}
                  className="h-8 w-8 p-0 flex-shrink-0 rounded-full bg-primary hover:bg-primary/90 transition-colors"
                  data-testid={`button-rate-${rating.id}`}
                >
              ...
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => handleRatePlace(rating, e)}
                    className="h-9 w-9 p-0 flex-shrink-0 rounded-full bg-primary hover:bg-primary/90 transition-colors"
                    data-testid={`button-rate-${rating.id}`}
                  >
                    <Plus className="h-4 w-4 text-white" />
                  </Button>
              ...
              <div className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={(e) => handleRatePlace(rating, e)}
                    className="h-9 px-4 text-xs bg-primary hover:bg-primary/90 transition-colors"
                    data-testid={`button-rate-${rating.id}`}
                  >
    ...
    <RatingBottomSheet
      ...
      restaurant={convertToRestaurant(selectedPlaceForRating)}
