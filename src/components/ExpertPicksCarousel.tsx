...
export function ExpertPicksCarousel() {
  ...
  const [selectedRestaurantForRating, setSelectedRestaurantForRating] = useState<PickedRestaurant | null>(null);
  ...
  const handleRateRestaurant = (restaurant: PickedRestaurant, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleRatingSubmit = (restaurantId: string, rating: number) => {
    ...
    // Convert PickedRestaurant to Restaurant format for adding to rated restaurants
    const ratedRestaurant = {
      name: selectedRestaurantForRating.name,
      address: '', // Not available in PickedRestaurant interface
      ...
      notes: `Rated ${rating}/10 from expert picks`,
      isWishlist: false,
      photos: [] as File[],
  ...
  const convertToRestaurant = (restaurant: PickedRestaurant): Restaurant | null => {
    if (!restaurant) return null;

    return {
      id: '', // Will be generated when added
      name: restaurant.name,
      address: '', // Not available in PickedRestaurant
          ...
          >
            ...
            <CardContent className="pt-0 pb-3">
              ...
              <Button
                variant="default"
                size="sm"
                onClick={(e) => handleRateRestaurant(place, e)}
                className="w-full h-8 text-xs bg-primary hover:bg-primary/90"
                data-testid={`button-rate-${place.place_id}`}
              >
