    ...
    const fetchRestaurantDetails = async () => {
      ...
      try {
        ...
        if (isValidUUID(restaurantId)) {
          ...
          // First try without user_id filter (RLS should handle permissions)
          let restaurantQuery = supabase
            .from('restaurants')
            .select('*')
            .eq('id', restaurantId);

          // Only add user_id filter if friendId is specified
          if (friendId) {
            restaurantQuery = restaurantQuery.eq('user_id', friendId);
          }
          // If no friendId, try without user_id filter first (RLS will handle access control)
          // This allows viewing your own restaurants without requiring user?.id to be set

          const { data: restaurantData, error: restaurantError } = await restaurantQuery.single();

          if (restaurantError) {
            console.error('Error fetching restaurant:', restaurantError);
            // If error and we have a user, try with user_id filter as fallback
            if (user?.id && !friendId) {
