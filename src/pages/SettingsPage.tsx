...
export default function SettingsPage({ onBack }: SettingsPageProps) {
  ...
  const handleConvertToExpert = async () => {
    ...
    try {
      ...
      setIsExpert(true);
      toast.success('Congratulations! You are now an expert. Your reviews will be highlighted with expert badges.');
    } catch (error: any) {
      console.error('Error converting to expert:', error);
      if (error.message.includes('duplicate key')) {
        toast.error('You are already an expert!');
        setIsExpert(true);
      } else {
        toast.error('Failed to convert to expert: ' + error.message);
      }
    } finally {
      setIsConvertingToExpert(false);
    }
  };
          ...
          <TabsContent value="account" className="space-y-6">
            ...
            <Card className={isExpert ? "border-amber-200 dark:border-amber-800 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20" : ""}>
              <CardHeader>
                ...
                <CardDescription>
                  {isExpert 
                    ? "You are a verified expert. Your reviews are highlighted with expert badges."
                    : "Become a verified expert to get special badges and features."
                  }
                </CardDescription>
              </CardHeader>
              <CardContent>
                {isExpert ? (
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-muted/50 rounded-lg">
                      <h4 className="font-medium mb-2">Expert Benefits:</h4>
                      <ul className="text-sm text-muted-foreground space-y-1">
                        <li>• Expert badge on your reviews and profile</li>
                        <li>• Your reviews appear in "Expert Picks" sections</li>
                        <li>• Special expert-only features and insights</li>
                        <li>• Higher visibility for your recommendations</li>
                      </ul>
                    </div>
                    <Button
