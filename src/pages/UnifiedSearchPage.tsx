...
export default function UnifiedSearchPage() {
  ...
  const { addRestaurant, updateRestaurant, restaurants, isLoading: restaurantsLoading } = useRestaurants();
  ...
  const [selectedRestaurantForDialog, setSelectedRestaurantForDialog] = useState<GooglePlaceResult | Restaurant | null>(null);
  ...
  // Handler for opening restaurant dialog with pre-filled data
  const handleAddRestaurant = (place: GooglePlaceResult) => {
    // Wait for restaurants to load before checking for duplicates
    if (restaurantsLoading) {
      toast.info('Loading your restaurants...');
      return;
    }

    console.log('🔍 Checking for duplicate restaurant:', place.name);
    console.log('📊 Current restaurants count:', restaurants.length);
    console.log('🍽️ All restaurants:', restaurants.map(r => ({ name: r.name, address: r.address, googlePlaceId: r.googlePlaceId })));
    
    // Check if restaurant already exists in user's ratings
    const existingRestaurant = restaurants.find(restaurant => {
      const nameMatch = restaurant.name.toLowerCase() === place.name.toLowerCase();
      const addressMatch = restaurant.address === place.formatted_address;
      const placeIdMatch = restaurant.googlePlaceId === place.place_id;
      ...
      console.log(`🔍 Checking ${restaurant.name}:`, {
        ...
        restaurantPlaceId: restaurant.googlePlaceId,
        placeId: place.place_id
      });
    ...
    if (existingRestaurant) {
      // Restaurant already exists, open it for editing
      toast.info(`You've already rated ${place.name}. Opening for editing...`);
      setSelectedRestaurantForDialog(existingRestaurant);
      setIsRestaurantDialogOpen(true);
    } else {
      // New restaurant, proceed with normal flow
      console.log('🆕 No existing restaurant found, proceeding with new restaurant flow');
      setSelectedRestaurantForDialog(place);
      setIsRestaurantDialogOpen(true);
    }
  };

  // Handler for saving restaurant from dialog
  const handleSaveRestaurant = async (restaurantData: any) => {
    console.log('💾 handleSaveRestaurant called with:', {
      restaurantName: restaurantData.name,
      selectedRestaurantId: (selectedRestaurantForDialog as Restaurant)?.id,
      selectedRestaurantName: selectedRestaurantForDialog?.name
    });
    
    try {
      // Type guard to check if selectedRestaurantForDialog is a Restaurant (has id property)
      const isRestaurant = (item: GooglePlaceResult | Restaurant | null): item is Restaurant => {
        return item !== null && 'id' in item && typeof item.id === 'string';
      };

      // Check if this is an existing restaurant being edited
      const isEditingExisting = isRestaurant(selectedRestaurantForDialog) && 
        restaurants.some(r => r.id === selectedRestaurantForDialog.id);

      console.log('🔍 Edit check:', {
        isEditingExisting,
        selectedRestaurantId: isRestaurant(selectedRestaurantForDialog) ? selectedRestaurantForDialog.id : null,
        hasId: isRestaurant(selectedRestaurantForDialog)
      });

      if (isEditingExisting && isRestaurant(selectedRestaurantForDialog)) {
        // Update existing restaurant
        console.log('✏️ Updating existing restaurant');
        await updateRestaurant(selectedRestaurantForDialog.id, restaurantData);
        toast.success("Restaurant rating updated!");
      } else {
        // Add new restaurant
      ...
      setSelectedRestaurantForDialog(null);
    } catch (error) {
  ...
  return <div className="w-full">
    ...
    <AlertDescription>{searchError}</AlertDescription>
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-2 flex-shrink-0">
                            ...
                            <Button size="sm" className="h-6 w-6 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium" onClick={e => {
                      e.stopPropagation();
      ...
      {/* Restaurant Dialog for Adding/Editing Restaurants */}
      <RestaurantDialog
        isOpen={isRestaurantDialogOpen}
        onOpenChange={setIsRestaurantDialogOpen}
        restaurant={selectedRestaurantForDialog ? (
          // Check if this is an existing restaurant (has an id)
          'id' in selectedRestaurantForDialog ? 
            // Existing restaurant - use the existing data
            selectedRestaurantForDialog as Restaurant :
            // New restaurant - create the data structure from GooglePlaceResult
            {
              name: selectedRestaurantForDialog.name,
              address: 'formatted_address' in selectedRestaurantForDialog ? selectedRestaurantForDialog.formatted_address : '',
              city: 'formatted_address' in selectedRestaurantForDialog ? (selectedRestaurantForDialog.formatted_address?.split(', ')[0] || '') : '',
              cuisine: 'Restaurant', // Default cuisine, user can change
              priceRange: 'price_level' in selectedRestaurantForDialog ? selectedRestaurantForDialog.price_level : 1,
              rating: undefined, // User will set this
              reviewCount: 'user_ratings_total' in selectedRestaurantForDialog ? selectedRestaurantForDialog.user_ratings_total : 0,
              notes: '',
              isWishlist: false,
              photos: [],
              michelinStars: null,
              // Google Places specific data
              googlePlaceId: 'place_id' in selectedRestaurantForDialog ? selectedRestaurantForDialog.place_id : undefined,
              website: undefined,
        ...
        dialogType={'id' in (selectedRestaurantForDialog || {}) ? "edit" : "add"}
