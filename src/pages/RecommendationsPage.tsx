...
export function RecommendationsPage({ restaurants, onAddRestaurant }: RecommendationsPageProps) {
  ...
  // Load initial recommendations instantly with optimized caching
  useEffect(() => {
    if (!hasLoadedInitial) {
      ...
      // Load cached data instantly without waiting - OPTIMIZED
      const loadInstantly = async () => {
        try {
          // Try to load from cache first
          const cachedData = localStorage.getItem('recommendations_cache');
          if (cachedData) {
            ...
            if (parsed && parsed.data && parsed.timestamp) {
              ...
              if (cacheAge < 3600000) { // 1 hour
                setAllLoadedRecommendations(parsed.data);
                setRecommendations(parsed.data.slice(0, 20));
                // Load fresh data in background after showing cache
                setTimeout(() => loadFreshDataInBackground(defaultCities), 100);
        ...
        // If no cache or cache expired, load fresh data immediately
        await loadFreshDataInBackground(defaultCities);
      };
  ...
  const loadCachedDataInstantly = (cities: string[]) => {
    // Try main cache first for instant loading
    try {
      const mainCache = localStorage.getItem('recommendations_cache');
      if (mainCache) {
      ...
      console.warn('Main cache failed, trying city cache');
    }
    
    // Fall back to city-specific cache
  ...
  const loadFreshDataInBackground = async (cities: string[]) => {
    ...
    try {
      ...
      if (allNewRecs.length > 0) {
        ...
        // Cache the new data in city-specific cache
        results.forEach((cityResults, index) => {
          if (cityResults.length > 0) {
        ...
        // Save to main cache for instant loading next time
        try {
          ...
          console.warn('Failed to save to main cache');
        }
      }
