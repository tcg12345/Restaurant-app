...
export function DiscoverPage() {
  ...
  const [selectedRestaurantForRating, setSelectedRestaurantForRating] = useState<RestaurantResult | null>(null);
  ...
  const handleRateRestaurant = (restaurant: RestaurantResult) => {
  ...
  const handleRatingSubmit = (restaurantId: string, rating: number) => {
    ...
    // Convert RestaurantResult to Restaurant format for adding to rated restaurants
    const ratedRestaurant = {
      name: selectedRestaurantForRating.name,
      address: selectedRestaurantForRating.address,
      city: selectedRestaurantForRating.location?.city || 'Unknown',
      cuisine: selectedRestaurantForRating.cuisine,
      priceRange: selectedRestaurantForRating.priceRange,
      rating: rating,
      reviewCount: selectedRestaurantForRating.reviewCount,
      googleMapsUrl: selectedRestaurantForRating.googleMapsUrl,
      notes: `Rated ${rating}/10`,
      isWishlist: false,
      latitude: selectedRestaurantForRating.location?.lat,
      longitude: selectedRestaurantForRating.location?.lng,
      website: selectedRestaurantForRating.website,
      phoneNumber: selectedRestaurantForRating.phoneNumber,
      photos: [] as File[],
      michelinStars: selectedRestaurantForRating.michelinStars,
      openingHours: selectedRestaurantForRating.openingHours,
    };
  ...
  const convertToRestaurant = (restaurantResult: RestaurantResult): Restaurant | null => {
    ...
    return {
      ...
      city: restaurantResult.location?.city || 'Unknown',
      cuisine: restaurantResult.cuisine,
      priceRange: restaurantResult.priceRange,
      rating: 0, // Will be set when rated
      notes: '',
      photos: [],
      isWishlist: false,
      userId: '',
      latitude: restaurantResult.location?.lat,
      longitude: restaurantResult.location?.lng,
      website: restaurantResult.website,
      phone_number: restaurantResult.phoneNumber,
      michelinStars: restaurantResult.michelinStars,
      openingHours: restaurantResult.openingHours,
      country: restaurantResult.location?.country,
